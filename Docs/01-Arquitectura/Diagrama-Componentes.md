...
* `POST /api/usuario/password/solicitar-verificacion` 🔒 — Ya no cambia la contraseña directo (reemplaza al antiguo `PUT /api/usuario/password`). Si la cuenta ya tiene una contraseña local, exige y verifica `passwordActual`; si se registró solo con Google/Facebook y todavía no tiene ninguna, no exige nada. En ambos casos manda un correo de verificación (mismo `IEmailService`/enlace que usa "olvidé mi contraseña") y el cambio real de contraseña se completa recién en `POST /api/password/restablecer`, una vez que la persona hace clic en el enlace — así el cambio de contraseña SIEMPRE pasa por verificación por correo, autenticado o no.
...
* `POST /api/password/recuperar` — Genera un token de restablecimiento y le manda por correo (`IEmailService`) un enlace a `/restablecer-password?token=&correo=` del frontend. Responde siempre el mismo mensaje genérico, exista o no ese correo (no revela si una cuenta existe). Requiere credenciales SMTP reales en `Smtp:*` de appsettings.json — sin eso, el token se genera pero el correo nunca sale (ver [[MEJORAS]]).
* `POST /api/password/restablecer` — Confirma el cambio de contraseña con ese token (hacer clic en el enlace del correo es lo que "verifica" al usuario antes de este paso).
